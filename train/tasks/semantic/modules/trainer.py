...
class Trainer():
    def __init__(self, ARCH, DATA, datadir, logdir, path=None):
        ...
        if self.ARCH["train"]["initialize_seed"]:
            seed = self.ARCH["train"]["seed"]
        ...
        self.parser = parserModule.Parser(root=self.datadir,
                                          ...
                                          rgb_image_width=self.ARCH["backbone"]["rgb_image_width"],
                                          rgb_image_height=self.ARCH["backbone"]["rgb_image_height"],
                                          gt=True,
                                          shuffle_train=True)
        ...
        if torch.cuda.is_available() and torch.cuda.device_count() > 0:
            if not self.ARCH["train"]["initialize_seed"]:
    ...
    def train_epoch(self, train_loader, model, criterion, optimizer, epoch, evaluator, scheduler, color_fn, report=10,
        ...
        for i, (in_vol, proj_mask, proj_labels, _, path_seq, path_name, _, _, _, _, _, _, _, _, _, proj_rgb, proj_fusion_labels,
                image) in enumerate(train_loader):
            ...
            # compute output
            if self.use_rgb_image:
                output = model([in_vol, image], proj_mask)
            else:
                output = model(in_vol, proj_mask)
            loss = criterion(torch.log(output.clamp(min=1e-8)), proj_labels.long())
    ...
    def validate(self, val_loader, model, criterion, evaluator, class_func, color_fn, save_scans):
        ...
        with torch.no_grad():
            ...
            for i, (
            in_vol, proj_mask, proj_labels, _, path_seq, path_name, _, _, _, _, _, _, _, _, _, proj_rgb,
            proj_fusion_labels, image) in enumerate(val_loader):
                if not self.multi_gpu and self.gpu:
                    in_vol = in_vol.cuda()
                    proj_mask = proj_mask.cuda()
                    if self.use_rgb_image:
